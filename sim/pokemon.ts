...
export class Pokemon {
	...
	*/
	...
	getMoveRequest(lockedMove?: string | null, restrictData?: boolean): RequestMoveSlot[] {
		...
		const moves = [];
		const zMoves = this.battle.canZMove(this);
		let hasValidMove = false;
		for (const [index, moveSlot] of this.moveSlots.entries()) {
			let moveName = moveSlot.move;
			const move = this.battle.getMove(moveName);
			const zMove = zMoves && zMoves[index] && zMoves[index]!.move ? this.battle.getMove(zMoves[index]!.move) : null;
