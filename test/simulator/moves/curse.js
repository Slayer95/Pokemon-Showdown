...
describe('XY/ORAS Curse targetting when becoming Ghost the same turn', function () {
	...
	[0, 1, 2].forEach(function (cursePos) {
		it('should target an opponent in Triples even if the user is on position ' + cursePos, function () {
			...
			assert.strictEqual(p1team.findIndex(function (set) {return set.species === 'Kecleon';}), cursePos);

			battle.join('p1', 'Guest 1', 1, p1team);
			battle.join('p2', 'Guest 2', 1, p2team);

			assert(battle.p1.pokemon.length === 3);
			assert(battle.p2.pokemon.length === 3);

			battle.commitDecisions();
			assert(battle.started);

			assert(battle.p1.active.length === 3);
			assert(battle.p2.active.length === 3);
			assert(battle.p1.active[cursePos]);
